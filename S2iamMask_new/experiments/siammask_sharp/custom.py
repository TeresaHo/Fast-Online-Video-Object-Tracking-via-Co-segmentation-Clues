...
class Refine(nn.Module):
    ...
    def forward(self, f, corr_feature, pos=None, test=False):
        if test:
            ...
            #print("f0 size") b,64,125,125
            #print("f1 size") b,256,63,63
            #print("f2 size") b,512,31,31
            ...
            p2 = F.unfold(f[2], (15, 15), padding=0, stride=1).permute(0, 2, 1).contiguous().view(-1, 512, 15, 15) # -1 : 578 2*17*17
            if not (pos is None): p2 = torch.index_select(p2, 0, pos)
        
        # corr_feature size b,256,25,25
        if not(pos is None):
            p3 = corr_feature[:, :, pos[0], pos[1]].view(-1, 256, 1, 1)
        else:
            p3 = corr_feature.permute(0, 2, 3, 1).contiguous().view(-1, 256, 1, 1) # b*25*25,256,1,1
        
        out = self.deconv(p3)
        
        # out           1250, 32, 15, 15
        # self.v2(p2)   578,32,15,15
        # self.h2(out)  1250,32,15,15
...
class Custom(SiamMask):
    def __init__(self, pretrain=False, **kwargs):
        ...
        #self.mask_fcn = MaskHEADRefine(256,28,255,1)
