...
parser.add_argument('--proposals', default=100, type=int, metavar='N',
...
def build_opt_lr(model, cfg, args, epoch):
    ...
    #opt_Adam  = torch.optim.Adam(trainable_params, lr=0.001, betas=(0.9, 0.99))
...
def main():
    ...
    if args.arch == 'Custom':
        ...
        #model = Custom(proposals=args.proposals,anchors=cfg['anchors'])
...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    for iter, input in enumerate(train_loader):
        ...
        x = {
            ...
            'gt_bbox' : torch.autograd.Variable(input[5]).cuda(),
        }
        
        outputs = model(x)

        rpn_mask_loss = torch.mean(outputs['losses'][0])
        ...
        loss = rpn_mask_loss*mask_weight
        ...
        if (iter + 1) % args.print_freq == 0:
            logger.info('Epoch: [{0}][{1}/{2}] lr: {lr:.6f}\t{batch_time:s}\t{data_time:s}'
                        ...
                        rpn_mask_loss=avg.rpn_mask_loss, mask_iou_mean=avg.mask_iou_mean,
