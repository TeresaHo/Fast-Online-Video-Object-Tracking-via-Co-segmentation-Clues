...
def bbox_transform_inv(anchors, deltas, batch_size):
    # anchors = [5*25*25,4]
    #batch_anchors = anchors.repeat(batch_size,1).view(batch_size,-1,4)
    ...
    dh = deltas[:, :, 3::4]
    """
    pred_ctr_x = delta[:,:,0] * batch_anchors[:,:, 2] + batch_anchors[:,:, 0]
    pred_ctr_y = delta[:,:,1] * batch_anchors[:,:, 3] + batch_anchors[:,:, 1]
    pred_w = torch.exp(delta[:,:,2]) * batch_anchors[:,:, 2]
    pred_h = torch.exp(delta[:,:,3]) * batch_anchors[:,:, 3]
    """
...
def clip_boxes(boxes, im_shape):
    ...
    boxes[:,:,0::4].clamp_(0, im_shape-1)
    boxes[:,:,1::4].clamp_(0, im_shape-1)
    boxes[:,:,2::4].clamp_(0, im_shape-1)
    boxes[:,:,3::4].clamp_(0, im_shape-1)
